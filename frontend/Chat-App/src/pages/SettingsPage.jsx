  ...
  return (
    <div className="fixed inset-0 m-auto h-4/5 w-2/5 shadow-lg rounded-3xl flex flex-col gap-10 items-center bg-rose-100 p-4 text-black font-mono">
      <Badge
        ...
        badgeContent={
          <IconButton className="cursor-pointer" onClick={handleIconClick} disabled={isUpdatingProfile}>
            <CameraAltIcon fontSize='large' color='inherit'  />
            <input type="file" className="hidden" accept="image/*" onChange={handleSelectImage} ref={imageInputRef}/>
          </IconButton>
        }
      >
        <Avatar 
          alt={loggedInUser.username} 
          src={!isUpdatingProfile && (selectedImage || loggedInUser.profilePic || "/static/images/avatar/2.jpg")} 
